...
public partial class SaveMenuItemFormControl : ContentView
{
    ...
    private async Task SaveMenuItemAsync()
    {
        ...
        {
            await ErrorAlertAsync("Моля, попълнете Име и Описание на артикула.");
            return;
        }

        if (Item.SelectedCategories.Length == 0)
        {
            await ErrorAlertAsync("Моля, изберете категория за артикула.");
            return;
        }

        if (Item.Icon == DefaultIcon)
        {
            await ErrorAlertAsync("Моля, изберете Снимка за артикула.");
            return;
        }
